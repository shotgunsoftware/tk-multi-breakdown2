...
What this templated string does:
    ...
    2. Uses Shotgun token resolution to display and format the entity's description field
...
get_file_item(index)
    - This is a helper method to extract the ``FileItem`` data from the given index. The `FileItem` object will hold the Shotgun data associated with this index, that can be displayed in the view.
...
We can extend this example to use the hook's static method ``get_file_item``, to get the Shotgun data for this index to display certain Shotgun data:
...
If the ``FileItem`` object is retrieved for this index, display the user who created this Shotgun entity that the item is associated with. Notice that if a FileItem object was not found, we default to display the indexes DisplayRole data.
