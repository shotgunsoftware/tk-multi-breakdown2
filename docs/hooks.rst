...
What this templated string does:
    ...
    2. Uses Flow Production Tracking token resolution to display and format the entity's description field

.. _sg_token_res:

Flow Production Tracking Token Resolution and Formatting
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^


The Flow Production Tracking token resolution will convert the values defined like, ``{token}``, into formatted strings using Flow Production Tracking data. These token values can be defined using the following format::
...
get_file_item(index)
    - This is a helper method to extract the ``FileItem`` data from the given index. The `FileItem` object will hold the Flow Production Tracking data associated with this index, that can be displayed in the view.
...
We can extend this example to use the hook's static method ``get_file_item``, to get the Flow Production Tracking data for this index to display certain Flow Production Tracking data:
...
If the ``FileItem`` object is retrieved for this index, display the user who created this Flow Production Tracking entity that the item is associated with. Notice that if a FileItem object was not found, we default to display the indexes DisplayRole data.
...
Here, we are returning a tuple containing the template string defined in the simple hook_ui_config and the FileItem object's Flow Production Tracking data dictionary. The returned tuple will then be passed to the ``ViewItemDelegate`` class, which will process the template string with the provided Flow Production Tracking data and replace any ``{token}`` values in the template string with the Flow Production Tracking data. See :ref:`sg_token_res` for more details.
